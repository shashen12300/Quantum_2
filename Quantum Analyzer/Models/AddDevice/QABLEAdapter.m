...
@implementation QABLEAdapter

static QABLEAdapter * _sharedBLEAdapter = nil;
...
{
    @synchronized([QABLEAdapter class])
    {
...
- (int) controlSetup: (int) s{
    ...
    self.CM = [[CBCentralManager alloc] initWithDelegate:self queue:nil options:@{CBCentralManagerOptionRestoreIdentifierKey:@"Quantum Analyzer"} ];
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central {
    ...
    if (central.state == CBCentralManagerStatePoweredOff) {
        self.addViewController.alertView.hidden = NO;
    }else if (central.state == CBManagerStatePoweredOn) {
        self.addViewController.alertView.hidden = YES;
    }
}
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral {
    ...
    if (configViewController) {
        [configViewController onConnected:YES];
    }
//    if(dvController)
//    {
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    if (!error) {
        ...
        if (service.characteristics.count == 1) {
            CBCharacteristic *c = [self getCurrentCharacteristic];
            if (c) {
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    if (!error) {
        ...
        if (beginViewController) {
            [beginViewController OnReadChar:characteristic];
        }
//        if(cvController)
//        {
...
- (void)startCheck {
    ...
    NSData *data = [self dataFromHexString:@"BB"];
    [self.activePeripheral writeValue:data forCharacteristic:[self getCurrentCharacteristic] type:CBCharacteristicWriteWithoutResponse];
}
/** 暂停检测*/
- (void)pauseCheck {
    NSData *data = [self dataFromHexString:@"CC"];
    [self.activePeripheral writeValue:data forCharacteristic:[self getCurrentCharacteristic] type:CBCharacteristicWriteWithoutResponse];
}
/* 继续检测*/
- (void)continueCheck {
    NSData *data = [self dataFromHexString:@"DD"];
    [self.activePeripheral writeValue:data forCharacteristic:[self getCurrentCharacteristic] type:CBCharacteristicWriteWithoutResponse];
}

/** 停止检测*/
- (void)stopCheck {
    NSData *data = [self dataFromHexString:@"EE"];
    [self.activePeripheral writeValue:data forCharacteristic:[self getCurrentCharacteristic] type:CBCharacteristicWriteWithoutResponse];
}
...
- (CBCharacteristic *)getCurrentCharacteristic {
    CBPeripheral *pheral = [QABLEAdapter sharedBLEAdapter].activePeripheral;
    NSArray *services = pheral.services;
    CBService *service = services[1];
